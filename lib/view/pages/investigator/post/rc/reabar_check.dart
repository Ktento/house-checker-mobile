...
class RebarDangerSurveyFormPage extends StatelessWidget {
  final String? uuid;
  const RebarDangerSurveyFormPage({super.key, this.uuid});
  ...
  Widget build(BuildContext context) {
    ...
    final List<Map<String, dynamic>> surveyItems1 = [
      {
        'title': '損傷度Ⅲ以上の損傷部材',
        'record': record.content.hasSevereDamageMembers,
        'options': ['A.無し', 'B.あり', ''],
      },
      {
        'title': '隣接建築物・周辺地盤',
      ...
      {
        'title': '地盤破壊による傾斜',
        'record': record.content.groundFailureInclination,
      ...
      {
        'title': '柱の損傷度Ⅴ',
        'record': record.content.percentColumnsDamageLevel5,
        'options': ['A.1%以下', 'B.1%～10%', 'C.10%超'],
      },
      {
        'title': '柱の損傷度Ⅳ',
        'record': record.content.percentColumnsDamageLevel4,
    ...
    final List<Map<String, dynamic>> surveyItems2 = [
      ...
      {
        'title': '外装材（モルタル・タイル・石貼り）',
        'record': record.content.exteriorMaterialMortarTileStone,
        'options': ['A.ほとんど無被害', 'B.部分的なひび割れ、隙間', 'C.顕著なひび割れ、剥離'],
      },
      {
        'title': '外装材（ALC・PC・金属・ブロック）',
        'record': record.content.exteriorMaterialALCPCMetalBlock,
                    ...
                    onPressed: () async {
                      ...
                      if (uuid != null) {
                        inevestigatorUpdateRecord(
                            rebarRecord: viewModel.rebarRecord, uuid: uuid!);
                      } else {
                        inevestigatorSendRecord(
                            rebarRecord: viewModel.rebarRecord);
                      }
                    },
